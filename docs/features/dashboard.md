...
O smoke real foi ampliado para os quatro períodos e as cinco leituras, mas requer `GR_SMOKE_*` no ambiente local. É possível defini-las no processo ou em `.env.smoke.local`, arquivo opcional ignorado pelo Git e carregado por `npm run smoke:local`. Sem essas credenciais, sua execução não pode ser declarada como provada. A vitrine foi revisada visualmente em 1440, 1280 e 1024 px, com estados preenchido, vazio, erro parcial e período personalizado. A revisão da rota autenticada com barra lateral e o smoke real continuam pendentes antes de declarar a fase pronta para aprovação.
