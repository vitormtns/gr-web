...
@Component({
    ...
    <section class="access">
      <div class="form-wrap">
        ...
        <form [formGroup]="form" (ngSubmit)="submit()" novalidate>
          ...
          <gr-input label="Senha" type="password" placeholder="Digite sua senha" [required]="true" [revealable]="true" [error]="passwordError()" formControlName="password" />
  ...
  styles:[`
    main{min-height:100dvh;display:grid;grid-template-columns:minmax(32rem,1.45fr) minmax(27rem,1fr);background:var(--color-bg)}
    ...
    .access{display:grid;align-items:center;padding:var(--space-8);background:var(--color-bg)}.form-wrap{width:min(100%,25rem);margin:auto;animation:page-enter var(--duration-context) var(--ease-emphasized) both}.mobile-brand{display:none;color:var(--color-primary)}.mobile-brand .brand-mark{border-color:var(--color-border);background:var(--color-primary-subtle)}
    .heading{margin-bottom:var(--space-6)}.heading h1{margin-bottom:var(--space-2);font-size:1.875rem;letter-spacing:-.04em}.heading p{margin:0}
    ...
    .support{margin:var(--space-6) 0 0;text-align:center;font-size:.8125rem;color:var(--color-text-secondary)}
    @media(max-width:52rem){main{grid-template-columns:1fr}.territory{display:none}.access{min-height:100dvh;padding:var(--space-6)}.mobile-brand{display:flex;margin-bottom:var(--space-12)}}
  `],
