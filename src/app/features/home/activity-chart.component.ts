...
@Component({
  ...
  template: `@if(mode()==='empty'){
    <div class="empty-pulse" role="status">
      <span aria-hidden="true"></span>
      <div>
        <strong>Nenhuma atividade registrada neste período</strong>
        <small>Novos fatos operacionais aparecerão aqui automaticamente.</small>
      </div>
    </div>
  } @else if(mode()==='sparse'){
    <ol class="sparse-events" aria-label="Eventos no período">
      @for(event of events(); track event.date + event.label){
        <li>
          <time [attr.datetime]="event.date">{{formatDate(event.date)}}</time>
          <i [style.background]="event.color" aria-hidden="true"></i>
          <div>
            <strong>{{event.label}}</strong>
            <span>{{event.value}} {{event.value===1?'registro':'registros'}}</span>
          </div>
        </li>
      }
    </ol>
    ...
    <div class="legend" role="group" aria-label="Séries da atividade">
      @for(series of definitions; track series.key){
        <button
          type="button"
          [class.selected]="selected().includes(series.key)"
          [attr.aria-pressed]="selected().includes(series.key)"
    ...
    <div class="chart-wrap">
      <svg viewBox="0 0 800 176" preserveAspectRatio="none" role="img" tabindex="0" [attr.aria-label]="description()" (keydown)="onKey($event)" (mousemove)="onPointer($event)" (mouseleave)="focused.set(null)" (focus)="focused.set(focused() ?? 0)">
        <defs>
          <filter id="dot-glow" x="-20%" y="-20%" width="140%" height="140%">
            <feDropShadow dx="0" dy="1" stdDeviation="2" flood-opacity=".25" />
          </filter>
        </defs>
        ...
        @for(series of visible(); track series.key; let seriesIndex = $index){
          ...
          @for(bucket of buckets; track bucket.date; let bucketIndex = $index){
            @if(bucket[series.key] > 0){
              <line
                [attr.x1]="x(bucketIndex)"
                [attr.y1]="laneY(seriesIndex)"
                [attr.x2]="x(bucketIndex)"
                [attr.y2]="laneY(seriesIndex) - pulseHeight(bucket[series.key])"
                [attr.stroke]="series.color"
                class="pulse-stem"
              />
              <circle
                [attr.cx]="x(bucketIndex)"
                [attr.cy]="laneY(seriesIndex) - pulseHeight(bucket[series.key])"
                r="4"
      ...
      @if(focused() !== null && buckets[focused()!]){
        <div class="chart-tooltip" role="status">
          ...
          @for(series of visible(); track series.key){
            <span>
              <i [style.background]="series.color"></i>
              {{series.label}}
              <b>{{buckets[focused()!][series.key]}}</b>
            </span>
          }
        </div>
      }
  ...
  <table class="sr-only">
    <caption>Atividade diária da fazenda no período</caption>
    <thead>
      <tr>
        <th scope="col">Data</th>
        @for(series of definitions; track series.key){ <th scope="col">{{series.label}}</th> }
      </tr>
    </thead>
    <tbody>
      @for(bucket of buckets; track bucket.date){
        <tr>
          <th scope="row">{{formatDate(bucket.date)}}</th>
          @for(series of definitions; track series.key){ <td>{{bucket[series.key]}}</td> }
        </tr>
      }
    </tbody>
  </table>`,
  styles: [`
    ...
    .legend i, .chart-tooltip i {
      width: 0.45rem;
      height: 0.45rem;
      display: inline-block;
      border-radius: 50%;
    }
    .legend button:not(.selected) i {
      opacity: 0.35;
    }
    .chart-wrap {
      position: relative;
      ...
      background: linear-gradient(180deg, rgba(235, 245, 238, 0.45) 0%, rgba(255, 255, 255, 0.8) 100%);
    }
    .chart-wrap svg {
      width: 100%;
      height: 100%;
    ...
    .empty-pulse > span {
      ...
      border-radius: 50%;
      box-shadow: 0 0 0 0.45rem rgba(18, 84, 52, 0.06);
    }
    .empty-pulse strong, .empty-pulse small { display: block; }
    .empty-pulse strong { font-size: 0.8125rem; color: var(--color-text); }
    .empty-pulse small { margin-top: 0.15rem; color: var(--color-text-muted); font-size: 0.72rem; }
    .sparse-events {
      display: grid;
    ...
    .sparse-events li:hover { background: var(--color-surface-soft); }
    .sparse-events li + li { border-left: 1px solid var(--color-border); }
    .sparse-events time {
      color: var(--color-text-muted);
      font-size: 0.6875rem;
      font-weight: 600;
    ...
    .sparse-events i { width: 0.55rem; height: 0.55rem; border-radius: 50%; }
    .sparse-events strong, .sparse-events span { display: block; }
    .sparse-events strong { font-size: 0.78rem; font-weight: 700; color: var(--color-text); }
    .sparse-events span { color: var(--color-text-secondary); font-size: 0.6875rem; }
    .sparse-note {
      margin: 0.45rem 0 0;
      color: var(--color-text-muted);
      font-size: 0.65rem;
    ...
    @media (max-width: 42rem) {
      .sparse-events { grid-template-columns: 1fr; }
      .sparse-events li + li { border-top: 1px solid var(--color-border); border-left: 0; }
...
export class ActivityChartComponent {
  ...
  readonly definitions: SeriesDefinition[] = [
    { key: 'births', label: 'Nascimentos', color: '#287d50' },
    { key: 'movements', label: 'Movimentações', color: '#326b86' },
    { key: 'healthTreatments', label: 'Tratamentos', color: '#a15f0b' },
    { key: 'weights', label: 'Pesagens', color: '#7057a8' },
    { key: 'sales', label: 'Vendas', color: '#816147' },
    { key: 'deaths', label: 'Mortes', color: '#b23a3a' },
    { key: 'breedings', label: 'Coberturas', color: '#637a49' },
    { key: 'calvings', label: 'Partos', color: '#547c80' },
  ];
  ...
  events(): ActivityEvent[] {
    return this.buckets.flatMap(bucket =>
      this.definitions
        .filter(series => bucket[series.key] > 0)
        .map(series => ({ date: bucket.date, label: series.label, value: bucket[series.key], color: series.color }))
    );
  }
  mode(): 'empty' | 'sparse' | 'pulse' {
    const count = this.events().length;
    return count === 0 ? 'empty' : count <= 3 ? 'sparse' : 'pulse';
  }
  description(): string {
    return `Gráfico de atividade. Séries visíveis: ${this.visible().map(item => item.label).join(', ') || 'nenhuma'}. Use as setas para percorrer as datas; a tabela acessível contém todos os valores.`;
  }
  toggle(key: SeriesKey): void {
    this.selected.update(items => items.includes(key) ? items.filter(item => item !== key) : [...items, key]);
  }
  ...
  onKey(event: KeyboardEvent): void {
    if (!['ArrowLeft', 'ArrowRight', 'Home', 'End'].includes(event.key) || !this.buckets.length) return;
